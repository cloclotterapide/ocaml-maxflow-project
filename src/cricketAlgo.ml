...
let rec get_win team_name = function
  |[] -> failwith "%s no team with this name " 
  |team::rest -> if team.name = team_name then team.win else get_win team_name rest 
...
(*let test = get_remaining "coco" [{team1="coco";team2="n";remaining=3};{team1="c";team2="coco";remaining=1};{team1="t";team2="c";remaining=1};{team1="team_name";team2="coco";remaining=0}]*) 
  ...
  let rec add_teams graph selected_team game_list team_list = match team_list with
    | [] -> graph
    | team::rest -> 
      if team.name = selected_team then 
        graph
      else
        let graph = new_node graph team.id in
        let graph = add_arc graph team.id (-2) ((get_win  selected_team team_list ) + (get_remaining selected_team game_list) - (get_win team.name team_list)) in
        add_teams graph selected_team game_list rest
...
let rec add_games graph selected_team = function
  |[] -> graph
  |{team1 = team1;team2= team2;remaining= rm}::rest -> if team1 = selected_team ||  team2 = selected_team then graph else
...
let cricketGraph cricket_data = 
  ...
  let graph = add_teams graph selected_team game_list team_list in
  let steam = get_team_from_name selected_team team_list in 
  let graph = add_games graph steam game_list in
  add_game_team_arcs graph steam team_list game_list
